...
from utility import hamming_scoreCV, hamming_get_accuracy, feature_correlation_sum, get_max_label_correlations, get_index_sum
import sklearn
...
class Star:
    ...
    def updateFitness(self,constant1, X, Y):
        ...
        self.fitness, self.ham_score, self.ham_loss = self.Obj_fun(constant1, X,Y) #set this to objective function
  
    def Obj_fun(self, constant1, X, Y):
        #print("inside Pbjective Function")
        feature_index = self.select_features()
        #print("feature index = ", feature_index)
        score, ham_score, ham_loss = self.get_score(constant1,feature_index, X, Y)
        #print("score = ", score)
    ...
    def get_score(self, constant1, feature_index, X, Y):
...
def fit(constant1,num_of_samples,num_iter, X, Y):
    ...
    while it < max_iter:
        ...
        for i in range(0, pop_number):
            ...
            pop[i].updateFitness(constant1,X, Y)
            pop[i].isBH = False
...
if __name__ == "__main__":
    ...
    #constants range 
    worst_features, best_fitness, ham_score, ham_loss = fit(0.01, 10,25,X_train,Y_train)
    global_max = 0
