...
from utility import weighted_label_correlations,weighted_label_correlations_70_30,hamming_scoreCV, hamming_get_accuracy, feature_correlation_sum, get_max_label_correlations, get_index_sum, get_max_label_correlations_gen, get_max_corr_label
import sklearn
...
def fit(constant1,num_of_samples,num_iter, X, Y):
    ...
    #label_dict = weighted_label_correlations(X,Y)
...
if __name__ == "__main__":
    ...
    print("\n\n---with feature selection------\n\n")
    worst_features, best_fitness, ham_score, ham_loss = fit(0.05, 15,25,X,Y)
    X_final= X.drop(X.columns[worst_features], axis = 1)
